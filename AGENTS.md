...
Run `aca --help` for the check list. Designs are accepted — read them before
changing anything:
...
- [docs/design/instruction-corpus.md](docs/design/instruction-corpus.md) — agent-context evidence library (ACA-0023): discovery, cascades, token accounting
- [docs/design/check-agent-context-cost.md](docs/design/check-agent-context-cost.md) — value per token of instruction files, protected tribal knowledge
...
2. **Interfaces are frozen as designed.** Checks talk to JudgeClient, never a
   ...
   No build step and no `dist/` — native type stripping only.
