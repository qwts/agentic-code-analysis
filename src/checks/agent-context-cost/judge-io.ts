...
import type { InstructionSource, SessionLoadSet } from '../../check-groups/agent-context/corpus/index.ts';
import type { TokenEstimator } from '../../check-groups/agent-context/corpus/index.ts';
...
export interface AgentContextCostVerdict extends FileVerdict {
  ...
  bytes?: number;
  bindings?: { tool: string; convention: string; activation: string; semantics: string }[];
  loadSets?: { id: string; baselineTokens: number; conditionalTokens: number }[];
...
const scopeLabel = (dir: string): string => (dir === '' ? 'repository root' : dir);

export function userPrompt(source: InstructionSource, loadSets: SessionLoadSet[]): string {
  const bindings = source.bindings
    .map(
      (b) =>
        `- ${b.tool} (${b.convention}), scope: ${scopeLabel(b.scopeDir)}, activation: ${b.activation} [${b.semantics.status}]` +
        (b.pathGlobs ? ` globs: ${b.pathGlobs.join(', ')}` : ''),
    )
    .join('\n');
  const sets = loadSets.map((s) => `- ${s.id}: baseline ~${s.baselineTokens}, conditional ~${s.conditionalTokens}`).join('\n');
  return `Instruction file: ${source.path} (${source.id})
Estimated cost: ~${source.estimate.tokens} tokens (${source.estimate.bytes} bytes; ${source.estimate.estimator} — reference estimate, not a billing claim)
Tool bindings:
${bindings || '(none)'}
...
export function judgeOutcome(
  ...
  for (const f of judged.findings) {
    ...
    const saved = Math.max(0, estimator.estimate(f.excerpt).tokens - estimator.estimate(f.replacement).tokens);
    findings.push({
