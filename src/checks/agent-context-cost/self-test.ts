...
import { referenceEstimator, type InstructionSource, type SessionLoadSet } from '../../check-groups/agent-context/corpus/index.ts';
import {
  achievedLevel,
  matchExpectation,
  qualifies,
...
/** Fixtures are judged as root Claude memory: one verified always-on
 * binding, so the exam isolates the value-density judgment from cascade
 * mechanics. */
function sourceOf(fixture: CalibrationFixture, content: string): { source: InstructionSource; sets: SessionLoadSet[] } {
  const estimate = referenceEstimator.estimate(content);
  const source: InstructionSource = {
    id: `repo:${fixture.file}`,
    origin: 'repository',
    path: fixture.file,
    content,
    ...
    bindings: [
      {
        ...
        semantics: { status: 'verified', source: DOCS, verifiedAt: '2026-08-04' },
      },
    ],
    diagnostics: [],
  };
  const sets: SessionLoadSet[] = [
    {
      ...
      tool: 'claude-code',
      targetDir: '',
      entries: [{ sourceId: source.id, activation: 'always', tokens: estimate.tokens, verified: true }],
      baselineTokens: estimate.tokens,
      conditionalTokens: 0,
      manualTokens: 0,
      complete: true,
    },
...
export async function selfTest(client: JudgeClient): Promise<GradedSelfTestResult> {
  ...
  for (const id of levelIds) {
    ...
    const verdicts = await mapPool(fixtures, CONCURRENCY, async (fixture) => {
      const { source, sets } = sourceOf(fixture, contentOf(fixture.content)!);
      const result = await client.judge({ system, user: userPrompt(source, sets), schema: VERDICT_SCHEMA, maxTokens: MAX_TOKENS });
      return judgeOutcome(source, result, referenceEstimator).verdict;
    });
    let passed = true;
