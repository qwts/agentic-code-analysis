...
// against the merge-base once per run with rename detection. Forked from
// context-footprint and trimmed — no growth line, line counts are footprint
// orientation and irrelevant to naming truth. The judge sees snapshots; git
...
export function buildComparisons(repoRoot: string, baseRef: string, files: string[]): Map<string, Prepared> {
  ...
  for (const file of files) {
    ...
    prepared.set(file, { ok: true, comparison: { kind: 'legacy', base, head } });
  }
  return prepared;
}
