...
// input"): import paths and imported-by paths, per snapshot — the check's
// only mechanical derivation (no export inventory; the judge identifies the
// in-scope surface from the full content). Paths only, never contents.
// Forked from context-footprint (ACA-0003 D1 — checks never import each
// other); git snapshot assembly lives in comparison.ts.
 ...
 * per-file lookups are then O(1) instead of O(changed × repo) sync I/O.
 */
