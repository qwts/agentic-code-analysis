// The naming-truth check: orchestrates rule text, per-file comparisons, the
 ...
 * import edges — plus rule, prompt version, provider, model. The path is a
 * semantic component here: the module name is itself judged evidence
 * (ACA-0014). Base ref/SHA and derived orientation stay out, so a moving
 * merge-base cannot re-bill an unchanged semantic pair.
 */
...
async function run(context: CheckContext): Promise<FileVerdict[]> {
  ...
  // dedupe so one path cannot race the pool and bill twice (ACA-0013).
