...
// with named cumulative qualification levels — not ENG-0151 routing tiers. A
// level names what a judge run demonstrated on this exact check + prompt
// version + fixture suite + provider + model; routing stays configuration.
...
import { ASSESSMENTS, CRITERIA, type ContextFootprintVerdict } from './judge-io.ts';
...
export function validateManifest(raw: unknown, contentOf: (file: string) => string | undefined): CalibrationManifest {
  ...
  if (typeof manifest !== 'object' || manifest === null || Array.isArray(manifest)) fail('must be an object (schemaVersion 2), not the v1 array');
 ...
 * residual expectation is met only by a residual finding — blocking
 * violations cannot stand in — and the matching residual must carry nonblank
 * evidence and suggestion (a bare label is not a detection). */
export function matchExpectation(expect: Expectation, verdict: ContextFootprintVerdict): boolean {
  const criteria = verdict.violations.map((violation) => violation.criterion);
  const residuals = verdict.residualViolations ?? [];
  return (
    verdict.assessment === expect.assessment &&
    verdict.verdict === expect.verdict &&
    (!expect.criteriaAnyOf || criteria.some((criterion) => expect.criteriaAnyOf!.includes(criterion))) &&
    (!expect.residualCriteriaAnyOf ||
      residuals.some(
        (residual) => expect.residualCriteriaAnyOf!.includes(residual.criterion) && nonblank(residual.evidence) && nonblank(residual.suggestion),
      ))
  );
}
 ...
 * version, rule text, the manifest itself, and every referenced fixture
 * content in manifest order. Any change produces a new identity, so recorded
 * evidence cannot be mistaken for a later exam.
 */
export function suiteIdentity(promptVersion: string, rule: string, manifestText: string, contents: string[]): string {
  const hash = createHash('sha256');
  for (const part of [promptVersion, rule, manifestText, ...contents]) hash.update(part).update('\0');
