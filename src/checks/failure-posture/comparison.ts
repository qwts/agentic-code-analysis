...
// snapshots; git stays in this module. Check-local fork of the
// context-footprint shape (ACA-0003 D1) — this is the only failure-posture
// module that invokes git.
import { execFileSync } from 'node:child_process';
import { readFileSync } from 'node:fs';
import { join } from 'node:path';
import { ConfigError } from '../../core/config.ts';
import { buildImporterIndex, CODE_EXT, importedBy, importsOf, readContents } from './import-graph.ts';
...
export function buildComparisons(repoRoot: string, baseRef: string, files: string[]): Map<string, Prepared> {
  ...
  for (const path of [...status.deleted, ...status.modified, ...status.renamedFrom.values()]) {
    ...
    if (content === undefined) {
      ...
      // this check's own swallowed-failure criterion, applied to itself.
