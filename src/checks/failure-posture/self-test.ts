...
// miss and would only add spend. Fixtures bypass the prefilter by design:
// calibration exercises the judge, and the fixtures are all candidates.
...
import { classifyFile } from './prefilter.ts';
...
function describe(verdict: FailurePostureVerdict): string {
...
function describeExpectation(expect: Expectation): string {
  return [
    ...
    expect.criteriaAllOf ? `[all of: ${expect.criteriaAllOf.join(', ')}]` : '',
    expect.residualCriteriaAnyOf ? `[residual any of: ${expect.residualCriteriaAnyOf.join(', ')}]` : '',
    expect.residualCriteriaAllOf ? `[residual all of: ${expect.residualCriteriaAllOf.join(', ')}]` : '',
  ]
    .filter(Boolean)
    .join(' ');
}
...
export async function selfTest(client: JudgeClient): Promise<GradedSelfTestResult> {
  ...
  for (const id of levelIds) {
    ...
    const verdicts = await mapPool(fixtures, CONCURRENCY, async (fixture) => {
      ...
      const hints = {
        head: classifyFile(comparison.head.path, comparison.head.content).hints,
        ...(comparison.kind === 'legacy' ? { base: classifyFile(comparison.base.path, comparison.base.content).hints } : {}),
      };
      const result = await client.judge({ system, user: userPrompt(comparison, hints), schema: VERDICT_SCHEMA, maxTokens: MAX_TOKENS });
