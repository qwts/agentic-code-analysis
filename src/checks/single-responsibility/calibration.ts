...
// Check-local fork bound to this check's criteria and assessments; the
// fixtures' expectations are any-of oracles (each names one criterion), so
// the fork carries no all-of variant. No filesystem access — the self-test
...
import { ASSESSMENTS, CRITERIA, type SingleResponsibilityVerdict } from './judge-io.ts';
 ...
 * violations cannot stand in. */
export function matchExpectation(expect: Expectation, verdict: SingleResponsibilityVerdict): boolean {
