...
test('self-test passes when fixtures judge as expected, fails on a miss', async () => {
  const byFixture = (request: JudgeRequest): JudgeResult => {
    const { user } = request;
    const dualActorViolation = { criterion: 'multiple-actors', evidence: 'e', suggestion: 's' };
    if (user.includes('Kind: new')) {
      return user.includes('SEASONAL_PROMO_PERCENT')
        ? { ok: true, verdict: { ...HELD_VERDICT, assessment: 'new-violating', head_violations: [dualActorViolation] } }
        : { ok: true, verdict: { ...HELD_VERDICT, assessment: 'new-compliant' } };
    }
    if (user.includes('shrank from 17 to 14'))
      return { ok: true, verdict: { ...HELD_VERDICT, assessment: 'regressed', head_violations: [dualActorViolation] } };
    if (user.includes('unchanged at 14'))
      return { ok: true, verdict: { ...HELD_VERDICT, assessment: 'held', head_violations: [dualActorViolation] } };
  ...
  // A held dual-actor file that fails to name the known residual debt is a
  // miss: the graded fixture requires the residual criteria, not just the pass.
  const noResidual = await check.selfTest!({
    provider: 'stub',
    model: 'm',
    judge: async (r) => {
      if (r.user.includes('unchanged at 14')) return { ok: true, verdict: { ...HELD_VERDICT, assessment: 'held' } };
      return byFixture(r);
    },
  });
  assert.equal(noResidual.passed, false);
  assert.ok(noResidual.lines.some((l) => l.startsWith('MISS legacy-held-dual')));

  const alwaysClean = await check.selfTest!({
    provider: 'stub',
    model: 'm',
    judge: async (r) =>
      r.user.includes('Kind: new')
        ? { ok: true, verdict: { ...HELD_VERDICT, assessment: 'new-compliant' } }
        : { ok: true, verdict: HELD_VERDICT },
  });
  assert.equal(alwaysClean.passed, false, 'the dual-actor fixtures are the negative control');
  assert.ok(alwaysClean.lines.some((l) => l.startsWith('MISS new-dual-actor')));
  assert.ok(alwaysClean.lines.some((l) => l.startsWith('MISS legacy-dual-back')));
});
